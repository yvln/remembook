...
passport.use(
  'local-signup',
  new LocalStrategy({
        // these are the names of the fields for login and password in
    ...
    (req, login, password, done) => {
      User
        ...
        .catch((err) => {
          ...
          return done(null, false, { message: ''}); // signals that signup was unsuccessful
        });
    })
);
...
passport.use(
  ...
  (req, login, password, done) => {
    User
      .findBylogin(login) // Returns a promise!
      .then((user) => {
        if (user) {
          ...
          if (isAuthed) {
            ...
            return done(null, false, { message: ''}); // signals we aren't logged in
          }
        } else {
          return done(null, false, { message: '' }); // signals we aren't logged in (no user)
        }
      });
  })
);
